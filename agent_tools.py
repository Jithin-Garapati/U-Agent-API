...
# Create cache directory for persistent storage - REMOVE THIS GLOBAL
# CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data_cache')
# if not os.path.exists(CACHE_DIR):
#     os.makedirs(CACHE_DIR, exist_ok=True)

# Create a global data cache to store DataFrames (memory index of what's on disk) - KEEP THIS GLOBAL FOR NOW
# Although ideally, this should also be session-specific if multiple sessions run in parallel *within the same process*.
# For simplicity with current structure, we keep it global but rely on unique data_ids per session.
DATA_CACHE = {}

# Cache management settings
CACHE_MAX_AGE_SECONDS = 3600  # 1 hour expiration

def _clean_expired_cache(cache_dir: str): # Add cache_dir argument
    """Remove cache files older than CACHE_MAX_AGE_SECONDS"""
    current_time = time.time()
    if not os.path.exists(cache_dir): # Check if session cache dir exists
        return
    for filename in os.listdir(cache_dir):
        file_path = os.path.join(cache_dir, filename) # Use cache_dir argument
        if os.path.isfile(file_path) and filename.endswith(".pkl"): # Check extension
            # Get file creation/modification time
            file_time = os.path.getmtime(file_path)
            if current_time - file_time > CACHE_MAX_AGE_SECONDS:
                try:
                    os.remove(file_path)
                    print(f"Removed expired cache file: {filename} from {cache_dir}") # Update print
                    ...
                    print(f"Error removing cache file {filename} from {cache_dir}: {e}") # Update print

def _get_cache_path(cache_dir: str, data_id: str) -> str: # Add cache_dir argument
    """Get the file path for a cache entry"""
    return os.path.join(cache_dir, f"{data_id}.pkl") # Use cache_dir argument

def _save_to_cache(cache_dir: str, data_id: str, data: Any) -> bool: # Add cache_dir argument
    """Save data to cache file and memory index"""
    try:
        # Ensure session cache directory exists
        os.makedirs(cache_dir, exist_ok=True)

        # Clean expired cache files occasionally (using the specific session cache dir)
        if uuid.uuid4().int % 10 == 0:  # ~10% chance to trigger cleanup
            _clean_expired_cache(cache_dir)

        # Save to memory index (still global for now)
        DATA_CACHE[data_id] = {'timestamp': time.time(), 'in_memory': True, 'cache_dir': cache_dir}

        # Save DataFrame to disk in the session cache dir
        cache_path = _get_cache_path(cache_dir, data_id)
        ...
        if isinstance(data, dict) and '_dataframe' in data:
            ...
            print(f"{GREEN}Saved DataFrame to cache: {data_id} in {cache_dir}{RESET}") # Update print
            return True
        ...
        print(f"{RED}Error saving to cache ({cache_dir}): {e}{RESET}")
        return False

def _load_from_cache(cache_dir: str, data_id: str) -> Any: # Add cache_dir argument
    """Load data from cache file"""
    cache_path = _get_cache_path(cache_dir, data_id)

    # Check the global memory index first
    if data_id not in DATA_CACHE or DATA_CACHE[data_id].get('cache_dir') != cache_dir:
         print(f"{YELLOW}Data ID {data_id} not found in memory index for cache {cache_dir}.{RESET}")
         # Try loading from disk anyway, but this might indicate an inconsistency
         # Or we can just return None here if strict memory index check is desired: return None
         pass # Allow attempt to load from disk below
    ...
    try:
        ...
        # Ensure it's in memory index (in case server restarted)
        if data_id not in DATA_CACHE:
            DATA_CACHE[data_id] = {'timestamp': time.time(), 'in_memory': True, 'cache_dir': cache_dir}
        else: # Update timestamp if already there
            DATA_CACHE[data_id]['timestamp'] = time.time()

        print(f"{GREEN}Loaded DataFrame from cache: {data_id} from {cache_dir}{RESET}") # Update print
        return df
    except Exception as e:
        print(f"{RED}Error loading from cache ({cache_path}): {e}{RESET}")
...
def static_param_lookup(param_name: str) -> Dict[str, Any]:

    """Get the exact row of a specific static parameter by name."""
...
def topic_data(topic_name: str, cache_dir: str, fields: Optional[List[str]] = None, file_path: Optional[str] = None) -> Dict[str, Any]: # Add cache_dir argument
    """Retrieve data from a topic with minimal processing."""
    print(f"Getting data for topic: {topic_name}, fields: {fields}, cache: {cache_dir}") # Update print
    
    # Use the base CSV directory derived from cache_dir (assuming structure like .../temp_data/session/cache -> .../temp_data/session/csv_topics)
    session_base_dir = os.path.dirname(cache_dir)
    csv_topics_dir = os.path.join(session_base_dir, 'csv_topics')

    # Get the data (pass the correct csv_dir)
    result = get_data(topic_name, fields, file_path=file_path, csv_dir=csv_topics_dir)
    ...
    # Save the full result to the persistent SESSION cache
    cache_success = _save_to_cache(cache_dir, data_id, result) # Pass cache_dir
    ...
    # Add data_id to the response dictionary so the caller knows it
    simplified_response = {
        "success": True,
        "topic": topic_name,
        "rows": rows,
        "columns": columns,
        "file_path": result.get("file_path", None), # Pass original file path
        "preview": preview_data,
        "statistics": statistics,
        "data_id": data_id if cache_success else None, # Return data_id ONLY if saved
...
AVAILABLE_FUNCTIONS = {
    ...
    "static_param_lookup": static_param_lookup,
